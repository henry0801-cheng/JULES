...
class Strategy:
    def __init__(self, data_file, n_days, top_k, v_bar, trail_stop, bias_threshold, data=None):
        """
        初始化策略
        ...
        :param bias_threshold: 60日乖離率門檻 (60BIAS < bias_threshold)
    ...
    def run_backtest(self):
        ...
        for i in range(len(dates)):
            today = dates[i]
            # Warmup period: max(n_days, 60) for BIAS
            if i < max(self.n_days, 60):
            ...
            if is_rebalance_day:
                ...
                # 2. 篩選 BIAS60 < bias_threshold
                bias_vals = bias60.loc[today, liquid_tickers].dropna()
                bias_filtered_tickers = bias_vals[bias_vals < self.bias_threshold].index

                # 3. 排名: 過去 N 天漲幅
                # returns_n.loc[today]
                # 只看 filtered tickers
                candidates = returns_n.loc[today, bias_filtered_tickers].dropna()
...
class AntColonyOptimizer:
    def __init__(self, data_file):
        ...
        self.param_ranges = {
            ...
            'bias_threshold': [i/100 for i in range(10, 110, 10)] # 0.1 to 1.0 step 0.1 (10% to 100%)
        }
    ...
    def run(self):
        ...
        temp_strat = Strategy(self.data_file, 10, 5, 5, 0.1, 0.3)
...
if __name__ == "__main__":
    ...
    if choice == '2':
        ...
        try:
            ...
            bias_threshold = float(input("Enter BIAS_THRESHOLD (e.g., 0.3 for 30%): "))
